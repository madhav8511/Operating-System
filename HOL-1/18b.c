...
int main()
{
    ...
    // Ask the user to choose a record to lock
    int input;
    ...
    struct flock t;
    t.l_type = F_RDLCK;  // Set the lock type to read lock
    ...
    // Set a read lock on the chosen record
    fcntl(fd, F_SETLKW, &t);
    ...
    record_val++;  // Update record...

    // Print the updated value of the record
    printf("TThe value of the record now is: %d\n", record_val);
